...
class Sampler():
    ...
    def evaluate_likelihood(self, points):
        ...
        if self.vectorized:
            ...
            #KZ start
            raise RuntimeError("To make Cobaya-mpi happy, please don't use pool")
            #KZ end
            result = list(self.pool_l.map(self.likelihood, args))
        elif self.cobaya_mpi and (self.pool_l is None):
            # Calculate the chunk size and the remainder for handling uneven divisions
            n_elements = len(args)
            size = get_mpi_size()
            rank = get_mpi_rank()
            mpi  = get_mpi()
